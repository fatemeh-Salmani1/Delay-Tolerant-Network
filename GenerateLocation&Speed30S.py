...
# # Load the dataset
# data = pd.read_csv('Reduce6Hour.csv')
...
# for i, row in tqdm(data.iterrows(), total=total_iterations):
...
#     # Calculate the time intervals between 30-second intervals
#     time_intervals = pd.date_range(timestamp, periods=int(velocity*2), freq='30S')
...
# new_data.to_csv('4_TestReduce6Hour30S', index=False)
...
with open('2014-10-01.csv', 'r') as input_file, open('GenerateLocation&Speed30S.csv', 'w') as output_file:
    ...
    for line in tqdm(input_file, desc='Processing lines'):
        ...
        if time_diff > 0:
            num_positions = int(time_diff / 30) + 1  
        else:
            num_positions = 0
        ...
        if num_positions > 0:
            ...
            for i in range(1, num_positions+1):
                current_timestamp = prev_timestamp + pd.Timedelta(seconds=30*i)
